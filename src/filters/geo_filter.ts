...
import {
    ...
    FieldNameModifier,
    FieldKinds,
    isGeoBoundingBoxFilterInput,
    GeoSubFieldFilterValue,
    isGeoDistanceFilterInput,
    isGeoPolygonFilterInput
} from '../types';
...
const makeFilterForQuery = (filter: GeoSubFieldFilterValue, fieldName: string) => {
    if (isGeoBoundingBoxFilterInput(filter)) {
        const {inclusion, kind, ...values} = filter;

        return {
            geo_bounding_box: {[fieldName]: values}
        };
    } else if (isGeoDistanceFilterInput(filter)) {
        const {inclusion, kind, distance, ...values} = filter;

        return {
            geo_distance: {distance, [fieldName]: values}
        };
    } else if (isGeoPolygonFilterInput(filter)) {
        const {inclusion, kind, ...values} = filter;

        return {
            geo_polygon: {[fieldName]: values}
        };
    } else {
        throw new Error(`Failed to parse geo filter input: ${JSON.stringify(filter)}`);
    }
};
...
class MultiSelectFilter<Fields extends string> extends BaseFilter<Fields, IConfig, GeoFieldFilter> {
    ...
    public userState(): {
        ...
        fieldFilters?: FieldFilters<Fields, GeoFieldFilter>;
    } | void {
        ...
        const fieldFilters = Object.keys(this.fieldFilters).reduce((fieldFilterAcc, fieldName) => {
            const filter = this.fieldFilters[fieldName as Fields] as GeoFieldFilter;
        ...
        }, {} as FieldFilters<Fields, GeoFieldFilter>);
    ...
    public clearAllFieldFilters = () => {
        runInAction(() => {
            this.fieldFilters = {} as FieldFilters<Fields, GeoFieldFilter>;
            this.filteredCount = {} as CountResults<Fields>;
            this.unfilteredCount = {} as CountResults<Fields>;
        });
    };
    ...
    public get _shouldRunFilteredQueryAndAggs(): object {
        const fieldFilters = objKeys(this.fieldFilters).reduce((acc, fieldName) => {
            const subFields = this.fieldFilters[fieldName] as GeoFieldFilter;
            if (!subFields) {
                return {...acc};
            }
    ...
    public _addQueriesToESRequest = (request: ESRequest): ESRequest => {
        ...
        return objKeys(this.fieldConfigs).reduce((acc, fieldName) => {
            ...
            if (filter) {
                return objKeys(filter as GeoFieldFilter).reduce((newQuery, selectedValue) => {
    ...
    public _addCountAggsToEsRequest = (request: ESRequest, fieldToFilterOn?: string): ESRequest => {
        ...
        return objKeys(this.fieldConfigs || {}).reduce((acc, fieldName) => {
            ...
            const valuesToFilterOn = objKeys(filter as GeoFieldFilter);
